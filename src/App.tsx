  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <main className="flex-1 overflow-hidden">
        <div className={`h-full overflow-y-auto ${tab === 'embed' ? 'flex' : 'hidden'}`}><EmbedderPage lang={lang} /></div>
        <div className={`h-full overflow-y-auto ${tab === 'scan'    ? 'block' : 'hidden'}`}><ScannerPage  lang={lang} /></div>
        <div className={`h-full overflow-y-auto ${tab === 'library' ? 'block' : 'hidden'}`}><LibraryPage  lang={lang} /></div>
      </main>
